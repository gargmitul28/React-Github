...
const Form = ({
  ...
  const myProfile = async () => {
    const responseOne = await fetch("https://api.github.com/users/gargmitul28");
    const dataOne = await responseOne.json();
    setProfileOne(dataOne);
    const responseTwo = await fetch(
      "https://api.github.com/users/gargmitul28/repos"
    );
    const dataTwo = await responseTwo.json();
    setProfileTwo(dataTwo);
    const responseThree = await fetch(
      "https://api.github.com/users/gargmitul28/repos?per_page=8&sort=asc"
    );
    const dataThree = await responseThree.json();
    setProfileThree(dataThree);
    const responseFour = await fetch(
      "https://api.github.com/users/gargmitul28/followers"
    );
